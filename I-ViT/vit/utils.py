...
def adjust_learning_rate(args, optimizer, epoch, batch_idx, data_nums, type="cosine"):
    if epoch < args.warmup_epochs:
        ...
        if epoch < 20 + args.warmup_epochs:
            lr_adj = 1.
        elif epoch < 40 + args.warmup_epochs:
            lr_adj = 1e-1
...
def save_model(model, optimizer, epoch, args, best):
    ...
    if optimizer is not None:
        ...
        torch.save(
            {'net': model.state_dict(), 'epoch': epoch, 'best_acc':best},
            os.path.join(args.work_dirs, '{}.pth'.format(epoch)),
        )
def save_model2(model, optimizer, epoch, path, best):
    #os.system('mkdir -p {}'.format(args.work_dirs))
    if optimizer is not None:
        torch.save(
            {
